...
database_client = cosmos_client.get_database_client(COSMOS_DB_DATABASE)
container_client = database_client.get_container_client(COSMOS_DB_CONTAINER)
...
def TextAnalyzer(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if text:
        ...
        response_data = {
            ...
            "originalText": text #To recall
        }

        # Store analysis result in Cosmos DB
        container_client.create_item(body=response_data)
...
# =============================================================================
# Endpoint: /api/GetAnalysisHistory
...
def GetAnalysisHistory(req: func.HttpRequest) -> func.HttpResponse:
    ...
    Returns:
        func.HttpResponse: JSON object with count and results array
    """

    logging.info("GetAnalysisHistory API was called!")

    # -------------------------------------------------------------------------
    # Read optional 'limit' query parameter
    # -------------------------------------------------------------------------
    try:
    ...
    # -------------------------------------------------------------------------
    # Build response with count and results
    # -------------------------------------------------------------------------
    response_data = {
        "count": len(items),
        "results": items
